...
LavenderLark
    ...
    Configuration 1:
        ...
        {'Dropout': 0.43964536366622975, 'Dropout_1': 0.0758216470290124, 'Dropout_2': 0.001400747061314947, 'Dropout_3': 0.6493198181649702, 'Dropout_4': 0.4150808120046018, 'Dropout_5': 0.016709924040157476, 'Dropout_6': 0.4931677498249911, 'pool_size': 6}
    ...
    Configuration 2:
        ...
        {'Dropout': 0.7515989358585912, 'Dropout_1': 0.0022875475552546765, 'Dropout_2': 0.15238139135094422, 'Dropout_3': 0.6520426381717099, 'Dropout_4': 0.6703303656360295, 'Dropout_5': 0.25927053833755814, 'Dropout_6': 0.3322443089305966, 'pool_size': 1}
    ...
    Configuration 3:
        ...
        {'Dropout': 0.25621479609203374, 'Dropout_1': 0.9329676595279734, 'Dropout_2': 0.015820733303320067, 'Dropout_3': 0.8799081098235765, 'Dropout_4': 0.0057108126272679745, 'Dropout_5': 0.018673368888892497, 'Dropout_6': 0.3700299347471297, 'pool_size': 0}
    ...
    Configuration 4:
        ...
        {'Dropout': 0.6391264746020587, 'Dropout_1': 0.17673032302518737, 'Dropout_2': 0.0022875475552546765, 'Dropout_3': 0.5498729447762613, 'Dropout_4': 0.22308105241884285, 'Dropout_5': 0.2413813194480476, 'Dropout_6': 0.2950655273313857, 'pool_size': 3}
...
def model(x_train, y_train, x_test, y_test):
    ...
    do0 = Dropout({{uniform(0,1)}})(f0)

    d0s = 512
    ds0 = Dense(d0s, activation='relu')(do0)

    do1 = Dropout({{uniform(0,1)}})(ds0)
    ...
    kernel_size = 5
    filters = 32
    do2 = Dropout({{uniform(0,1)}})(ci)

    c0 = Conv1D(filters, kernel_size)(do2)

    # --- begin code ---

    do3 = Dropout({{uniform(0,1)}})(ds1)

    codesize = 64

    densecode = Dense(codesize, activation='relu')(do0)

    pool_size = {{choice([1,2,3,4,5,6,7,8])}}
    ...
    do4 = Dropout({{uniform(0,1)}})(codelayer)

    u0s = 256
    us0 = Dense(u0s, activation='relu')(do4)

    do5 = Dropout({{uniform(0,1)}})(us0)

    u1s = 256
    us1 = Dense(u1s, activation='relu')(do5)

    do6 = Dropout({{uniform(0,1)}})(us1)
